...
- `src/web/` — Hono HTTP server for the operator web surface (gateway-net only). `server.ts` (`buildOperatorApp`) wires all operator routes behind the browser guard (session + allowlist + CSRF). `src/web/operator/` contains the authenticated operator routes:
  ...
  - `idempotency.ts` — `createIdempotencyGuard`: bounded in-memory per-operator idempotency store. Keys are namespaced `${githubUserId}:${clientKey}`. Supports `check`, `record`, and `remove` (rollback on sync launch failure). Evicts expired entries before live ones at capacity.
  ...
  - `web-approval.ts` — `createWebAutoDenyApproval`: auto-deny approval transport for web runs. Immediately rejects every tool permission request so web runs never hold the repo lock waiting for interactive approval.
