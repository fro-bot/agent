...
# fired (writes /tmp/gateway-ready) AND PID 1 is alive. Cleared at process
# startup so a stale flag from a prior process cannot mask a current-run
# failure. A real liveness probe (HTTP /healthz) lands alongside the
# workspace agent.
HEALTHCHECK --interval=10s --timeout=3s --retries=12 --start-period=45s \
  CMD test -f /tmp/gateway-ready && kill -0 1 || exit 1
