...
**The workspace pin and the action default advance together.** The `harness-release` workflow opens an auto-PR that bumps both `ARG OPENCODE_VERSION` in `deploy/workspace.Dockerfile` and `DEFAULT_OPENCODE_VERSION` in `packages/runtime/src/shared/constants.ts` to the same new harness build. Merging that PR is the gate — not merging holds both surfaces at the prior build.

**To bump the workspace to a newer harness release:** merge the auto-PR opened by the `harness-release` workflow. Both the action default and the workspace image advance together.
...
- `ipc: host` — shares the host IPC namespace (shared memory, semaphores, message queues); combined with `SYS_PTRACE` or other capabilities it enables host-process inspection and lateral-movement within the host trust boundary. `ipc: service:<x>`, `ipc: container:<x>`, `ipc: shareable`, and `ipc: private` are not host-namespace escapes and are allowed.
- `userns_mode: host` — disables user-namespace remapping; when the Docker daemon runs with `--userns-remap`, container-root is normally mapped to an unprivileged host UID, but `userns_mode: host` opts out of that remapping, making container-root equivalent to host-root and amplifying any capability escape into a full host-root compromise.
- `uts: host` — shares the host UTS namespace (hostname and NIS domain name) with the container, disclosing the host identity; a compromised container can read (and with sufficient privilege write) the host hostname, leaking infrastructure topology and enabling hostname-based authentication bypasses.
- `cap_add: SYS_PTRACE` — grants `ptrace(2)` on host processes; combined with shared namespaces (e.g. `ipc: host` or `pid: host`) it enables host-process inspection and lateral-movement. The existing capability normalization catches `CAP_SYS_PTRACE` and lowercase variants.

**Forward constraint:** never give the workspace a gateway endpoint that performs caller-directed outbound requests. The gateway has two HTTP surfaces with distinct trust boundaries:

- **Announce surface** (sandbox-net reachable): the HMAC/replay-gated `POST /v1/announce` endpoint, which posts a fixed embed — it does not perform arbitrary outbound requests on behalf of the caller. This is the only workspace-reachable HTTP ingress.
