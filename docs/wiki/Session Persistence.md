...
On affected runs, the Actions cache can still restore state, but its write token is not equally capable. For `issue_comment` and `issues` runs initiated by actors without repository write access, GitHub injects a read-only `ACTIONS_RUNTIME_TOKEN` for the Actions cache. `@actions/cache` authenticates with that runner-injected token rather than `GITHUB_TOKEN`, so adding or changing a workflow `permissions:` block cannot make the cache writable; the permissions shown for `GITHUB_TOKEN` are irrelevant to this cache operation. `workflow_dispatch` and `schedule` runs are not affected by this trigger-specific token behavior.
