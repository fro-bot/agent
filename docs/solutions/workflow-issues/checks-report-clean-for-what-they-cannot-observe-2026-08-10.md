...
- [A check written from inside its own premise cannot fail](a-check-written-from-inside-its-own-premise-cannot-fail-2026-09-04.md) — the third adjacent mode. This doc covers checks that saw too little; that one covers checks that saw the right thing but shared a premise with it — visit order, mock state, which caller, what counts — so the check is structurally blind to that premise breaking.
