...
- [A check written from inside its own premise cannot fail](a-check-written-from-inside-its-own-premise-cannot-fail-2026-09-04.md) — the third adjacent mode. This doc covers a fully observed thing whose wrong property was read; that one covers a check that sees the right thing and the right property but was authored from the same assumption it exists to test, so the one failure it was written to catch is the one it cannot see.
