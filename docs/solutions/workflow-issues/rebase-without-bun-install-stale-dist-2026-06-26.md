...
applies_when:
  ...
  - 'Adding or removing a runtime dependency in ANY workspace package, even one not bundled into the committed dist/'
tags:
  - rebase
  - bun-install
  - dist
...
## Dependency adds regenerate the workspace-wide notices file

`dist/THIRD_PARTY_NOTICES.txt` is generated by `bun run build` from the **whole workspace** dependency tree, not just the packages bundled into the committed Action `dist/`. Adding a runtime dependency to any workspace package — including one that is never bundled into the Action's `dist/`, such as `packages/gateway` — still changes `dist/THIRD_PARTY_NOTICES.txt` and fails CI's committed-dist gate.

The instinct "this package isn't in `dist/`, so no rebuild is needed" is wrong specifically for the notices file, because it attributes the entire workspace lockfile, not just the bundled entry point's dependency graph.

After adding or removing any runtime dependency, anywhere in the workspace:

```bash
bun install --frozen-lockfile
bun run build
git add dist/THIRD_PARTY_NOTICES.txt
```

Commit the regenerated `dist/THIRD_PARTY_NOTICES.txt` even when the rest of `dist/` is unchanged.

For a fully deterministic rebuild that matches CI's clean-checkout output — useful when a first rebuild produced a wrong chunk hash from stale incremental `node_modules` — do a clean reinstall:
...
- After adding or removing a runtime dependency in any workspace package, even one that is not bundled into the committed Action `dist/` — the notices file still needs regenerating.
