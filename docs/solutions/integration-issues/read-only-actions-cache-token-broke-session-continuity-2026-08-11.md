...
- [Repair on restore must precede capture on save](../logic-errors/repair-before-capture-sqlite-session-cache-loop-2026-09-02.md) — the inverse failure in the same subsystem, twice over: first a save that succeeded and persisted state that poisoned every later run; then, after that fix, a save that succeeded and was unrestorable because the cache version hash diverged. Here the save was denied and reported as success.
