...
- **Known limit:** The marker only reaches the wire on text parts. Chat Completions reads it exclusively from text parts, and the Responses API's `output_text` serialization never emits it at all, so assistant turns contribute no anchor on that path. Placement targets the last text part rather than the last part of any type for this reason.
