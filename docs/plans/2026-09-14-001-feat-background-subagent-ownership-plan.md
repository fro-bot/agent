...
The consequence: **a dispatch whose event is never observed is not recovered by reconciliation.** The ledger cannot know about work it never saw, and reconciliation's own two inputs cannot tell that work apart from a foreground subagent. This is a real gap, not a solved problem, and the release gate must weigh it — it is bounded by the run deadline, and background dispatch is now enabled, so it is live exposure rather than latent.

**Correction (2026-09-19).** An earlier version of this paragraph said closing it needs an upstream discriminant on the session record. That was wrong, and the same wrong claim reached three code comments before a source check against the pinned base version disproved it. The session record genuinely carries no flag, but the discriminant exists elsewhere: `tool/task.ts` records `{parentSessionId, sessionId, background: true}` through `ctx.metadata()` before the background job starts, `session/tools.ts` stores it at the tool part's `state.metadata`, and `core/session/projector.ts` persists the whole part with only identifiers stripped. `GET /session/{id}/message` returns it, and the SDK types declare `metadata` on both running and completed tool states. So `part.tool === 'task' && part.state.metadata.background === true` distinguishes a background dispatch from a foreground delegation, which carries no `background` key at all.

Closing the gap therefore means reading persisted task parts, not waiting on upstream. Two things such a reader must handle: correlating by `part.messageID` and tool start time, so a resumed session's historical dispatches are not adopted as this invocation's; and keeping observation separate from execution — upstream's background-job registry is explicitly process-local, so a restarted server can learn a dispatch existed while being unable to recover its in-process status. Conflating those two is the most likely origin of the original mistake.
...
- **Weigh the dropped dispatch before flipping** (see Unit 3). A dispatch whose event is never observed is not recovered by reconciliation, whose two inputs cannot distinguish a background child session from a foreground one. The ledger reads zero and the run proceeds normally over work it does not know about. That is the plan's original central hazard, still open. It is bounded by the run deadline — and the flag is now enabled, so it is reachable rather than hypothetical. A persisted-task-part reader could close it without upstream changes (see the correction under Unit 3); nothing implements one yet, so the hazard stands.
...
| A dispatch whose event is never observed leaves the ledger reading zero while a child writes | **Open.** Reconciliation was meant to cover this and cannot: its two inputs carry no discriminant between a background child session and a foreground one, so adopting untracked children claimed ordinary delegation as owned work. A discriminant does exist on the persisted tool part (see the correction under Unit 3), but nothing reads it yet. Bounded by the run deadline, reachable now that dispatch is enabled, and a stated input to the release gate (Unit 14) |
