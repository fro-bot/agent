...
- **Archive via the v2 SDK client, before the fresh attempt.** Implementation reality (resolved during Unit 2): the v1 `SessionClient` the session module uses can **read** `time.archived` (its `Session` type carries it) but **cannot** typed-write it — v1 `session.update` body is `{title?}` only. The typed write path is the **v2** client (`createOpencodeClient` from `@opencode-ai/sdk/v2/client`, flat `update({sessionID, time:{archived}})`), constructed from the server `baseUrl`. So `archiveSession(baseUrl, sessionId, logger)` builds a loopback v2 client (the established `session-tools.ts` per-call pattern) — no cast, no suppression. Archiving preserves the audit trail and, combined with the resolver fix below, guarantees the overflowed session is never re-continued. Archive **before** the fresh attempt so a crash mid-recovery cannot re-wedge the next run on the known-overflowed session; the accepted cost is that a crash before the fresh verdict degrades continuity to a cold start next run (a wedged required check is worse than losing one PR's transcript continuity). If the fresh attempt also overflows, archive it too.
...
- Create: `archiveSession(baseUrl, sessionId, logger)` helper — builds a v2 `createOpencodeClient({baseUrl})` (from `@opencode-ai/sdk/v2/client`) and calls `session.update({sessionID, time:{archived}})` (v1 client cannot typed-write `time`) — colocate under `packages/runtime/src/session/`
...
- Recovery sequence: `archiveSession(serverHandle.server.url, overflowedId, logger)` (before the fresh attempt; `server.url` is the loopback baseUrl for the v2 archive client) → build recovery-prep (`forceFresh`, `excludeSessionIds: [overflowedId]`, small cap) → compute `remainingMs = timeoutMs − elapsed` (fail without restart if `<= 0`) → second `executeOpenCode` with the remaining budget → return the recovered result, tagged with `overflowRecovery`.
