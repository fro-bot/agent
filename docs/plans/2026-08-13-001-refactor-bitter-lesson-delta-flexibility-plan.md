...
- **R3 — No push credential to the model:** The conflict resolver child process uses the existing child-scoped environment scrubbing/allowlist pattern and receives only the model/provider authentication required to reach the configured model plus explicit non-secret runtime values. It receives no GitHub token, App private key, AWS/cloud credentials, browser/session auth, askpass helper, or inherited secret-bearing environment. Network egress is limited to the configured model endpoint and existing required package/service endpoints; it does not gain general authenticated GitHub egress. Public sources are fetched anonymously; an auth-required source fails closed.
- **R4 — Hermetic conflict-scope enforcement:** Each model-resolution attempt runs in a disposable isolated worktree/check-out rooted inside the integration workspace. The check-out is recreated from the exact pre-conflict commit before every attempt, then the same merge is reapplied to regenerate the conflict set; `git reset --hard` alone is insufficient because it does not remove ignored or untracked state. No symlink-following writes, repo-external writes, `.git`/git-metadata mutation, submodule/gitlink mutation, ignored files, or untracked artifacts are permitted unless a future explicit allowlist says otherwise. Post-turn validation compares repository root, changed/unmerged/untracked/ignored sets, and git modes. Accepted staged paths must be regular files from the allowed conflicted set, and code stages only explicit validated paths, never `git add -A`. Any boundary violation destroys the worktree and fails the attempt. Broader read-only context may be allowed after evidence, but write scope never widens implicitly.
...
- Richard Sutton, [*The Bitter Lesson*](http://www.incompleteideas.net/IncIdeas/BitterLesson.html) — the primary source for preferring methods that continue to exploit computation over fixed human knowledge.
- Anthropic, [*SWE-bench Sonnet*](https://www.anthropic.com/engineering/swe-bench-sonnet) — primary engineering evidence for scaffolds that preserve model judgment rather than hard-code a workflow.
- Anthropic, [*Building Effective Agents*](https://www.anthropic.com/engineering/building-effective-agents) — primary engineering guidance on simple composable patterns, routing, parallelization, and evaluator loops; this plan uses the principle conditionally, not as a claim that all orchestration is bad.
- Anthropic, [*Writing Tools for Agents*](https://www.anthropic.com/engineering/writing-tools-for-agents) — primary engineering guidance on tool ergonomics and context economy.
...
- **KTD3 — Treat each model checkout as hermetic and disposable.** Every attempt starts from the exact pre-conflict commit in a recreated isolated worktree rooted inside the integration workspace. The resolver may edit only regular files from the allowed conflict set; code owns symlink/file-mode/gitlink boundaries, staging, marker checks, verification, credentials, cleanup, and push.
- **KTD4 — Scope child credentials and egress explicitly.** The resolver uses the existing child-scoped environment scrubbing/allowlist pattern. It receives only configured model/provider auth and explicit non-secret runtime values, with model-endpoint and required package/service egress but no authenticated GitHub or cloud egress. All current integration sources are public; if a source unexpectedly requires authentication, fail closed instead of widening the model's environment or silently changing source authority.
...
- **AE3 — File and repository boundaries fail closed.** A resolver that follows a symlink, writes outside the worktree, mutates `.git` metadata, changes a submodule/gitlink, creates an ignored/untracked artifact, or changes a path outside the allowed conflict set is rejected; the worktree is destroyed and the attempt fails. Accepted staged paths are explicit regular files, never the result of `git add -A`.
...
- **AE5 — Resolver egress is bounded.** Denied GitHub/cloud/browser/session credential keys and askpass state are absent from the child environment; the model turn can reach only the configured model endpoint and required package/service endpoints, and cannot push or perform authenticated GitHub operations.
...
**Goal:** Give the model only the contextual repair work that cannot be made deterministic, with a bounded, credentialless, conflict-scoped contract.
...
- Modify: `.github/workflows/harness-integrate.yaml` only as required to pass model inputs to the code-owned driver while preserving the one-job/OIDC boundary.
...
- Treat every model-resolution attempt as a disposable isolated worktree/check-out rooted inside the integration workspace. Recreate it from the exact pre-conflict commit before every attempt, then reapply the same merge to regenerate the conflict set. Do not rely on `git reset --hard` alone: ignored, untracked, temporary, and metadata state must not survive an attempt.
- The resolver prompt contains the conflicted path set, conflict metadata, relevant read-only repository context, and the bounded objective: repair the conflict. It does not ask for a patch artifact, final shell transcript, push, or release summary.
- The model edits conflicted files directly with native file tools. Its child process uses the existing child-scoped environment scrubbing/allowlist pattern and receives only configured model/provider auth plus explicit non-secret runtime values. It receives no GitHub token, App private key, AWS/cloud credentials, browser/session auth, askpass helper, inherited secret-bearing environment, or source-auth fallback. Network egress is limited to the configured model endpoint and existing required package/service endpoints; it does not gain general authenticated GitHub egress and cannot push or perform authenticated GitHub operations.
- The initial allowed changed-path set is the conflicted set. After the turn, code compares the repository root, changed/unmerged/untracked/ignored sets, and git modes. It verifies no unmerged paths; no conflict markers in relevant text; no symlink-following or repo-external writes; no `.git`/git-metadata mutation; no submodule/gitlink mutation; no ignored or untracked artifacts; no unexpected rename/delete/type-change outside the allowed set; and no changed path outside the allowed conflict set. Accepted staged paths must be regular files from that set.
- If broader context is genuinely required, the first attempt records an out-of-scope request and fails validation. A bounded code/reviewer reassessment may widen read-only context for the second attempt, but write scope never widens implicitly and the model cannot self-authorize additional writable paths. Arbitrary out-of-scope changes are never accepted merely because the build passes.
- Any boundary violation destroys the disposable worktree and fails the attempt. The retry recreates a clean checkout from the exact pre-conflict commit and reapplies the same merge; it does not reuse the first attempt's filesystem.
- Code explicitly stages only validated regular-file paths, never `git add -A`, then performs merge completion, squash, workflow stripping, build, version verification, and provenance. The model never controls the index or push.
- Count resolver attempts per ref. Restore a deterministic conflict state before a retry; after two attempts, fail the integration hard.
...
- Happy: one conflicted file is repaired in place, all markers/unmerged paths disappear, only the conflicted regular file is explicitly staged, and the integration continues.
- Happy: a clean merge never constructs or invokes the model resolver.
- Edge: a resolver edits a second path; validation rejects it, destroys the worktree, and records the out-of-scope request.
- Edge: symlink-following, repo-external/temp-artifact, ignored/untracked, `.git`/git-metadata, submodule/gitlink, or unexpected file-mode mutations are each rejected before staging.
- Edge: rename, delete, and type-change conflicts are characterized; valid regular-file resolutions remain eligible only when they stay within the allowed conflict set, while non-regular outcomes are rejected under the current allowlist rather than silently staged.
- Edge: a conflict marker remains in a non-obvious marker form or an unmerged index entry remains; validation fails before staging/push.
- Error: resolver process fails, times out, or returns without repair; the ref receives at most one further attempt and then fails hard.
- Security: denied GitHub/cloud/browser/session secret keys, App private key, push credential, and askpass state are absent from the child environment; the model turn cannot push or perform authenticated GitHub operations; anonymous source fetch auth failure remains fail-closed.
- Integration: after attempt one leaves ignored/untracked/temp state, attempt two recreates the exact pre-conflict checkout and reapplies the merge without inheriting that state.
- Integration: a valid conflict resolution reaches build/version/provenance, while invalid resolution cannot reach push.

**Expected verification outcomes:** Only conflict turns invoke the model; no accepted model output contains unmerged paths, conflict markers, symlink/repository-boundary violations, metadata/gitlink changes, ignored/untracked artifacts, or unapproved paths; accepted staged paths are explicit regular files; denied secret keys are absent; no push/auth operation is possible from the model turn; the worktree is destroyed on boundary violation; attempt count never exceeds two per ref.
