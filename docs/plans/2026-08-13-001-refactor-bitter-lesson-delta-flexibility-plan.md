...
The operational delta does not favor deletion. Eager presearch costs two SDK calls and roughly 300 bytes of injected context per run, both deterministic. Disabling it did not remove that cost so much as relocate it: the candidate spent more output and reasoning tokens and 17-60% more wall-clock time performing its own retrieval through the native `session_*` tools. Durations and token counts are advisory and were observed at one sample per cell on a single small model, so this is directional evidence rather than a measurement.
