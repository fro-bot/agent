...
- [ ] **Unit 3h: Operator session OAuth token retention** *(prerequisite for repo authorization; added 2026-06-19 from Unit 4 design review)*
  ...
  **Approach:**
  ...
  - **#951 intersection — strictly-temporary shim (P1 from design review):** this stores a gateway-session-scoped token for the gateway's own `checkRepoAuthz`; it does not presume the gateway is the permanent operator-auth authority. To avoid becoming the duplicated operator-identity/token authority #951 warns against, 3h is explicitly a **temporary shim with a removal criterion**: when #951 resolves (dashboard-delegated or shared-auth), the gateway-held token retention is removed or replaced by consuming the chosen credential source, and `checkRepoAuthz` is reworked to take its token from there. The plan records this removal/migration path now so the shim does not become permanent path-dependence. Do not build token-refresh or long-lived-credential machinery here.
  ...
  **Verification:**
  - The operator OAuth token is retained server-side, accessible only through the narrow accessor, dropped on session end or on first detected expiry/revocation, scoped to repo-read, and never exposed to clients or logs; the removal criterion tied to #951 is documented.
...
- [ ] **Unit 3i: Server-owned run index for `runId → repo` resolution** *(prerequisite for SSE observation and web launch; added 2026-06-19 from Unit 4 design review)*
  ...
  **Approach:**
  ...
  - Resolution contract: `lookup(runId)` returns `{repo, surface}` for any run with durable run-state (live or recent), or `undefined` only when no run-state exists anywhere. A route that gets `undefined` returns the same generic not-authorized/not-found shape as an unauthorized run — it must not become a run-existence oracle. Eviction from the accelerator must NEVER cause `undefined` for a run whose run-state still exists.
