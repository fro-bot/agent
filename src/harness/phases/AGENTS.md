...
| **Routing**       | `routing.ts`       | Event parsing and trigger routing (70 L)       |
...
```
bootstrap → routing → acknowledge → session-prep → execute → finalize → cleanup
```
...
- **Graceful Skip**: Routing phase can mark a run as skipped; downstream phases are bypassed.
