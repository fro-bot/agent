...
export async function runResponsePost(params: RunResponsePostParams, logger: Logger): Promise<ResponsePostResult> {
  ...
  // when recovered from a fallback artifact: an approval is an endorsement the harness
  // cannot support for content that was not the agent's actual final output. The verdict
  // text itself, and every other finding, is preserved unchanged.
