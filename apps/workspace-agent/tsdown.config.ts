...
/** Known workspace-agent symbol that must be present in the produced bundle. */
const REQUIRED_SYMBOL = 'startWorkspaceAgent'

/**
 * Post-build guard: reads the produced `dist/main.mjs` and asserts it contains
 * the known workspace-agent symbol. Fails the build loudly if absent, catching
 * any future entry-resolution regression regardless of root cause.
 *
      ...
      if (symbolPresent === false) {
        throw new Error(
          `[bundle-symbol-guard] Expected symbol "${REQUIRED_SYMBOL}" not found in ${bundlePath}. ` +
            `The bundle may have resolved the wrong entry point (e.g. the repo-root action harness instead of apps/workspace-agent/src/main.ts). ` +
