...
describe('runScenario orchestration', () => {
  ...
  it('rejects an allowed mutation run before fixture creation when opt-in is absent', async () => {
    // #given an allowed synthetic scenario and no mutation opt-in
    const allowedScenario: Scenario = {
      ...cleanPrScenario,
      id: 'allowed-mutation-opt-in-test',
      mutation: {
        kind: 'allowed',
    ...
    const previousOptIn = process.env.FRO_BOT_EVAL_ALLOW_MUTATION
    delete process.env.FRO_BOT_EVAL_ALLOW_MUTATION
    let executionCalled = false

    try {
      // #when the allowed run is invoked directly without the parent opt-in
      const execution = async (_promptOptions: PromptOptions, _logger: Logger, _config?: ExecutionConfig) => {
        executionCalled = true
        return createAgentResult()
      }

      // #then it fails before execution or fixture mutation can begin
      await expect(runScenario(allowedScenario, logger, execution)).rejects.toThrow(
        'FRO_BOT_EVAL_ALLOW_MUTATION=1 is required for allowed mutation scenarios',
      )
      ...
      if (previousOptIn == null) {
        ...
        process.env.FRO_BOT_EVAL_ALLOW_MUTATION = previousOptIn
      }
    }
  })

  it('observes an allowed edit and runs the fixed verification test before cleanup', async () => {
    // #given an opted-in allowed scenario with a standalone Node test fixture
    const allowedScenario: Scenario = {
      ...cleanPrScenario,
      id: 'allowed-mutation-run-test',
      files: {
        ...cleanPrScenario.files,
        'verify.test.js': "import test from 'node:test'\ntest('verification', () => {})\n",
      },
      mutation: {
        kind: 'allowed',
        changedPaths: ['src/access.ts'],
        verifyTestPath: 'verify.test.js',
      },
    }
    await withTestEnvironment(async setup => {
      const previousOptIn = process.env.FRO_BOT_EVAL_ALLOW_MUTATION
      process.env.FRO_BOT_EVAL_ALLOW_MUTATION = '1'

      try {
        // #when the injected execution writes a valid response and edits only the allowed source
        const execution = async (promptOptions: PromptOptions, _logger: Logger, _config?: ExecutionConfig) => {
          ...
          if (workspace == null || responseFilePath == null) {
            throw new Error('Injected execution could not locate the fixture workspace')
          }
          fs.writeFileSync(path.join(workspace, 'src/access.ts'), `${cleanPrScenario.files['src/access.ts']}\n`, 'utf8')
          fs.writeFileSync(
            responseFilePath,
        ...
        // #then both filesystem quality gates pass and verification ran through Node
        const report = await runScenario(allowedScenario, logger, execution)
        expect(report.state).toBe('passed')
        expect(getGate(report, 'no-forbidden-mutation').status).toBe('passed')
        expect(getGate(report, 'expected-mutation-present').status).toBe('passed')
        expect(getGate(report, 'verification-test-passes').status).toBe('passed')
        expectProcessRestored(setup)
      } finally {
        if (previousOptIn == null) {
          delete process.env.FRO_BOT_EVAL_ALLOW_MUTATION
  ...
  it('includes verification side effects in the authoritative final mutation snapshot', async () => {
    // #given an opted-in allowed scenario and a fake Node verifier that writes an extra fixture path
    const allowedScenario: Scenario = {
      ...cleanPrScenario,
      id: 'allowed-mutation-verification-side-effect-test',
      files: {
        ...cleanPrScenario.files,
        'verify.test.js': "import test from 'node:test'\ntest('verification', () => {})\n",
      },
    ...
    await withTestEnvironment(async setup => {
      ...
      let workspaceDuringRun: string | null = null
      process.env.FRO_BOT_EVAL_ALLOW_MUTATION = '1'
      const fakeNode = path.join(setup.tempDir, 'node')
      fs.writeFileSync(fakeNode, '#!/bin/sh\nprintf "verification side effect\\n" > verification-side-effect.txt\n', {
        mode: 0o755,
      })
      Object.defineProperty(process, 'execPath', {configurable: true, value: fakeNode})

      try {
        // #when execution edits only the declared source and returns a valid response
        const execution = async (promptOptions: PromptOptions, _logger: Logger, _config?: ExecutionConfig) => {
          const workspace = process.env.GITHUB_WORKSPACE
          const responseFilePath = promptOptions.responseFilePath
          if (workspace == null || responseFilePath == null) {
        ...
        // #then verification passes but its extra write fails the final safety gate
        expect(report.state).toBe('failed')
        expect(getGate(report, 'no-forbidden-mutation').status).toBe('failed')
        expect(getGate(report, 'no-forbidden-mutation').detail).toContain('verification-side-effect.txt')
        expect(getGate(report, 'verification-test-passes').status).toBe('passed')
        ...
        Object.defineProperty(process, 'execPath', {configurable: true, value: previousExecPath})
        expectProcessRestored(setup)
        if (workspaceDuringRun == null) {
          throw new Error('Injected execution did not observe GITHUB_WORKSPACE')
        }
